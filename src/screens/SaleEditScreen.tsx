...
type Props = NativeStackScreenProps<RootStackParamList, 'SaleEdit'>;
...
export function SaleEditScreen({ navigation, route }: Props) {
  ...
  const [loaded, setLoaded] = React.useState<SaleEditRow | null>(null);

  React.useEffect(() => {
    const load = async () => {
      const db = await getDb();
      const row = await getSaleByIdForEdit(db, saleId);
      if (!row) {
        Alert.alert('Not found', 'Sale not found');
        navigation.goBack();
        return;
      }
      ...
      setSelectedVariantId(row.variant_id);
      setSoldPriceText(String(row.final_sold_price));
      setQty(row.quantity_sold);
      setIsReturn(row.is_return === 1);
      setNotes(row.notes ?? '');
      setSaleDate(new Date(row.sale_date));
    };
  ...
  const refreshVariants = React.useCallback(async () => {
    ...
    if (loaded && rows.some((v) => v.id === loaded.variant_id)) {
      setSelectedVariantId(loaded.variant_id);
    }
  }, [loaded, selectedProductId]);
  ...
  const onSave = React.useCallback(async () => {
    ...
    try {
      const db = await getDb();
      await updateSaleAndAdjustStock(db, {
        saleId,
        nextVariantId: selectedVariantId,
        nextFinalSoldPrice: price,
        nextQuantity: qty,
        nextSaleDate: saleDate.getTime(),
        nextIsReturn: isReturn,
        nextNotes: notes.trim() ? notes.trim() : null,
      });
  ...
  }, [isReturn, notes, navigation, qty, saleDate, saleId, selectedVariantId, soldPriceText]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.grid2}>
        <View style={{ flex: 1 }}>
          ...
          <TextInput value={soldPriceText} onChangeText={setSoldPriceText} keyboardType="numeric" style={styles.input} />
        </View>
